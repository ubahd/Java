...
public class ViewTaskDetails extends SBTS.Control{
    ...
    getTaskDetails2(bean, setbookID);
    gotoPage("/ViewTaskDetails.jsp", request, response); 
    
    }
   
private void getTaskDetails2(SBTS.Shared bean, String BookID) throws ServletException, IOException{
        String[][] taskdetails;
        SBTS.DBI dbi = null;
try{
    dbi = new SBTS.DBI();
        ...
        if(dbi.connect()){
        taskdetails = dbi.getTaskDetails2(BookID);
        bean.setTaskDetails2(taskdetails);
        } 
}
