...
public class DBI{
   ...
   /* The arrays below hold string values that are used by the getString method to retirve information from ResultSet objects */    
    ...
    String [] book = {"BookID", "Title", "StartDate", "ISBN", "BookStatus", "FileName", "BookFormat"};    
    ...
    String [] Task = {"TaskID", "BookID", "TaskType", "TaskNotes", "TaskStatus", "TechnicianID", };
    String [] Task2 = {"TaskID", "BookID", "TaskType", "TaskNotes", "TaskStatus", "TechnicianID", "EmpFirstName", "EmpLastName"};
    String [] Task3 = {"TaskID", "StartDate", "TaskType", "TaskStatus", "EmpFirstName", "EmpLastName"};
    String [] selecttask = {"TaskID","Title", "TaskType", "TaskStatus", "TaskNotes", "StartDate", "FileName"};
    String [] Prob = {"EmpID", "EmpFirstName", "EmpLastName", "BookID", "BookStatus", "Title", "TaskType"};
...
//Method to get the information of the selected contract   
    ...
    public String[][] getTaskDetails(String TaskID) throws SQLException{ 
    ResultSet rst = this.execQuery("SELECT t.TaskID, t.BookID, t.TaskType, t.TaskNotes, t.TaskStatus, t.TechnicianID, e.EmpFirstName, e.EmpLastName From Task t, Employees e WHERE TaskID = '"+TaskID+"' && t.TechnicianID = e.EmpID");
    int columns = 8;
    int records = RecordNum(rst);
    String temp;
    String [][] result = new String[records][columns];
    ...
    return result;
	 ...
	 //Method to get the details of every task assigned to a book for shepherd   
    public String[][] getTaskDetails2(String BookID) throws SQLException{ 
    ResultSet rst = this.execQuery("SELECT t.TaskID, t.BookID, t.TaskType, t.TaskNotes, t.TaskStatus, t.TechnicianID, e.EmpFirstName, e.EmpLastName From Task t, Employees e WHERE BookID = '"+BookID+"' && t.TechnicianID = e.EmpID");
    int columns = 8;
    int records = RecordNum(rst);
    String temp;
    String [][] result = new String[records][columns];
   ...
   //Method to get the list of designers
    ...
    ResultSet rst = this.execQuery("SELECT BookID, Title, StartDate, ISBN, BookStatus, FileName, BookFormat From Book WHERE BookStatus != 'Published' AND BookStatus != 'Task Assigned' AND ShepherdID = '"+empid+"'"); // needs more detailed select
    String temp;
    int records = RecordNum(rst);
    int columns = 7;
   ...
   //Method to get list of tasks that belong to a tech
    public String[][] getTechTaskList(int empid) throws SQLException{ 
  	ResultSet rst = this.execQuery("SELECT TaskID, BookID, TaskType, TaskNotes, TaskStatus, TechnicianID From Task WHERE TaskStatus != 'Task Complete' AND TaskStatus != 'Task Problem' AND TechnicianID = '"+empid+"'"); 
    String temp;
    int records = RecordNum(rst);
    int columns = 6;
    String [][] result = new String[records][columns]; // matrix to hold book list of data
	...
	//Method to update the task status and tasknote for task hold status
	 public String[][] getUpdatesn(String taskstatus, String tasknotes, String TaskID) throws SQLException {
		Statement stmt = conn.createStatement();
		String rst = "UPDATE Task SET TaskStatus = '"+taskstatus+"', TaskNotes = CONCAT(CURDATE(), ' ','"+tasknotes+"', ' ', TaskNotes) WHERE TaskID = '"+TaskID+"';";
		stmt.executeUpdate(rst);
		ResultSet rst2 = this.execQuery("SELECT t.TaskID, t.BookID, t.TaskType, t.TaskNotes, t.TaskStatus, t.TechnicianID, e.EmpFirstName, e.EmpLastName From Task t, Employees e WHERE TaskID = '"+TaskID+"' && t.TechnicianID = e.EmpID");
    		int columns = 8;
   		int records = RecordNum(rst2);
    		String temp;
    		String [][] result = new String[records][columns];
    		if(rst2.first())
    		{
...
//Method to update the task status and tasknote for task complete status
	 public String[][] getUpdatesnb(String taskstatus, String tasknotes, String TaskID, String BookID) throws SQLException {
		Statement stmt = conn.createStatement();
		String rst1 = "UPDATE Task SET TaskStatus = '"+taskstatus+"', TaskNotes = CONCAT(CURDATE(), ' ','"+tasknotes+"', ' ', TaskNotes) WHERE TaskID = '"+TaskID+"';";
		stmt.executeUpdate(rst1);
		String rst = "UPDATE Book SET BookStatus = '"+taskstatus+"' WHERE BookID = '"+BookID+"';";
		stmt.executeUpdate(rst);
		ResultSet rst2 = this.execQuery("SELECT t.TaskID, t.BookID, t.TaskType, t.TaskNotes, t.TaskStatus, t.TechnicianID, e.EmpFirstName, e.EmpLastName From Task t, Employees e WHERE TaskID = '"+TaskID+"' && t.TechnicianID = e.EmpID");
    		int columns = 8;
   		int records = RecordNum(rst2);
	 ...
	 public String[][] getProblem() throws SQLException {
		ResultSet rst = this.execQuery("select Employees.EmpID, Employees.EmpFirstName, Employees.EmpLastName, Book.BookID, Book.BookStatus, Book.Title, Task.TaskType from Employees Inner Join Book on Employees.EmpID = Book.ShepherdID Inner Join Task On Book.BookID = Task.BookID where Book.BookID = Task.BookID and Book.BookStatus = 'Task Problem' group by Book.ShepherdID"); 
    String temp;
    int records = RecordNum(rst);
    int columns = 7;
    String [][] result = new String[records][columns]; // matrix to hold book list of data
    ...
    ResultSet rst = this.execQuery("SELECT t.TaskID,  t.TaskType, t.TaskStatus, t.StartDate, e.EmpFirstName, e.EmpLastName From Task t, Employees e WHERE TaskID = (SELECT MAX(LAST_INSERT_ID(TaskID)) FROM Task) && t.TechnicianID = e.EmpID");
    int columns = 6;
    int records = RecordNum(rst);
    String temp;
    String [][] result = new String[records][columns];
    ...
    public void EditTask(String taskid, String tasknotes) throws SQLException{ 
    		Statement stmt = conn.createStatement();
		String rst = "UPDATE Task SET TaskNotes = '"+tasknotes+"' WHERE TaskID = '"+taskid+"';";
		stmt.executeUpdate(rst);
