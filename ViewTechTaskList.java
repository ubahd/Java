...
public class ViewTechTaskList extends SBTS.Control{
    protected DBI dbi;
   ...
   //Call the method that gets the lists of the books belonging to the shepherd
    getTechTaskList(bean, empid);
    gotoPage("/ViewTechTaskList.jsp", request, response); //Dispatch to ViewShepherdBookRecords.jsp to show the book list
    
    }
    
    //Method to get the list of books belonging to a shepherd
private void getTechTaskList(SBTS.Shared bean, int empid) throws ServletException, IOException{
	// create a DBI shell
        SBTS.DBI dbi = null;
...
try{
	...
	if(dbi.connect()){
        ...
        Tlist= dbi.getTechTaskList(empid);

	// save this list of books in the bean
        bean.setTechTaskList(Tlist);
        } 
} 
