...
The purpose of this servlet is to get the list of books from the database that belong to a shepherd and need their help.
   ...
   + This servlet dispatches to ViewTaskProblem.jsp
...
public class ViewTaskProblem extends SBTS.Control{
    protected DBI dbi;
   ...
   //Call the method that gets the lists of the books belonging to the shepherd
    getProblem(bean);
    gotoPage("/ViewTaskProblem.jsp", request, response); //Dispatch to ViewShepherdBookRecords.jsp to show the book list
    
    }
    
    //Method to get the list of books belonging to a shepherd
private void getProblem(SBTS.Shared bean) throws ServletException, IOException{
	// create a DBI shell
        SBTS.DBI dbi = null;
...
try{
	...
	if(dbi.connect()){
        ...
        Vprobs= dbi.getProblem();

	// save this list of books in the bean
        bean.setProblem(Vprobs);
        } 
} 
